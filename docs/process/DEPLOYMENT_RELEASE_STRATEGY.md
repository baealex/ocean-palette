...
- `/data`: database persistence. The container stores SQLite at `/data/db.sqlite3`.
- `/assets`: asset persistence. The container stores served image files under `/assets/images`.
- In Docker, the app uses `DATABASE_URL=file:/data/db.sqlite3` directly.
- In Docker, the app uses `OCEAN_PALETTE_IMAGE_DIR=/assets/images` directly.
- Docker storage must not depend on symlinks from app source directories into mounted volumes.
...
- `pnpm smoke:docker` builds or runs a Docker image and verifies app/API health, direct Docker storage env, DB creation under `/data`, and image upload under `/assets/images`.
...
- Source run image default: `packages/server/.env` sets `OCEAN_PALETTE_IMAGE_DIR="public/assets/images"`.
- Example file: `packages/server/.env.example`.
- Container default: `packages/server/Dockerfile` sets `DATABASE_URL=file:/data/db.sqlite3`.
- Container asset default: `packages/server/Dockerfile` sets `OCEAN_PALETTE_IMAGE_DIR=/assets/images`, so app URLs under `/assets/images` map to the mounted `./assets/images` directory directly.
