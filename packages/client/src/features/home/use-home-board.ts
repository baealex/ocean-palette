...
export const useHomeBoard = () => {
    ...
    const reorderCategory = useCallback(
        async (activeCategoryId: number, overCategoryId: number) => {
            ...
            const oldIndex = current.findIndex(
                (category) => Number(category.id) === activeCategoryId,
            );
            const newIndex = current.findIndex(
                (category) => Number(category.id) === overCategoryId,
            );
            ...
            const optimistic = arrayMove(current, oldIndex, newIndex).map(
                (category, index) => ({
                    ...category,
                    order: index + 1,
                }),
    ...
    const reorderKeyword = useCallback(
        async (
            categoryId: number,
            activeKeywordId: number,
            overKeywordId: number,
        ) => {
            ...
            const oldIndex = targetCategory.keywords.findIndex(
                (keyword) => Number(keyword.id) === activeKeywordId,
            );
            const newIndex = targetCategory.keywords.findIndex(
                (keyword) => Number(keyword.id) === overKeywordId,
            );
    ...
    const addKeywords = useCallback(
        async (categoryId: number, rawKeywords: string): Promise<boolean> => {
    ...
    const addKeywordSampleImage = useCallback(
        async (keywordId: number, imageFile: File): Promise<boolean> => {
            return runMutation(async () => {
    ...
    const removeKeywordSampleImage = useCallback(
        async (keywordId: number): Promise<boolean> => {
            return runMutation(async () => {
