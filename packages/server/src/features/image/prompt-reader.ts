...
import { extractPromptParts } from '~/features/live/live-images.utils';
...
function parseTextChunk(
    ...
    const keyword = data
        .toString('latin1', 0, separatorIndex)
        .trim()
        .toLowerCase();
    if (!keyword) {
        return null;
    ...
    const text = decodeBufferText(
        data.subarray(separatorIndex + 1),
        'utf8',
    ).trim();
    if (!text) {
        return null;
...
interface CollectPromptTextsInput {
    nodeId: string;
    nodeMap: Map<string, PromptGraphNode>;
    visited: Set<string>;
}

function collectPromptTexts({
    nodeId,
    nodeMap,
    visited,
}: CollectPromptTextsInput): string[] {
    ...
    for (const inputValue of inputValues) {
        ...
        texts.push(...collectPromptTexts({ nodeId: refId, nodeMap, visited }));
    }

    return texts;
}

function buildExifTextEntryMap(
...
function splitSamplerAndScheduler(
    ...
    for (const suffix of schedulerSuffixes) {
        ...
        if (normalized.endsWith(marker)) {
            return {
                sampler: sampler
                    .slice(0, sampler.length - marker.length)
                    .trim(),
                scheduler: suffix,
            };
        }
...
function parseA1111Metadata(rawParameters: string): Partial<ParsedImageMeta> {
    ...
    const upscaleSamplerData = splitSamplerAndScheduler(
        readLabelValue(parameters, 'Hires sampler') ||
            readLabelValue(parameters, 'Upscale sampler'),
        readLabelValue(parameters, 'Hires schedule type') ||
            readLabelValue(parameters, 'Upscale schedule type'),
    );
    ...
    return {
        ...
        upscaleSteps: toPositiveInteger(
            readLabelValue(parameters, 'Hires steps') ||
                readLabelValue(parameters, 'Upscale steps'),
        ),
        upscaleCfgScale: toFiniteNumber(
            readLabelValue(parameters, 'Hires CFG scale') ||
                readLabelValue(parameters, 'Upscale CFG scale'),
        ),
        upscaleSeed: toSeedString(
            readLabelValue(parameters, 'Hires seed') ||
                readLabelValue(parameters, 'Upscale seed'),
        ),
        upscaleFactor: toFiniteNumber(
            readLabelValue(parameters, 'Hires upscale') ||
                readLabelValue(parameters, 'Upscale by'),
        ),
        upscaler:
            readLabelValue(parameters, 'Hires upscaler') ||
            readLabelValue(parameters, 'Upscaler'),
        sizeWidth,
        sizeHeight,
        clipSkip: toPositiveInteger(readLabelValue(parameters, 'Clip skip')),
        vae: readLabelValue(parameters, 'VAE'),
        denoiseStrength: toFiniteNumber(
            readLabelValue(parameters, 'Denoising strength'),
        ),
    };
}
...
function parseComfyMetadata(rawPrompt: string): Partial<ParsedImageMeta> {
    ...
    for (const [nodeId, node] of Array.from(nodeMap.entries())) {
        ...
        if (classType.includes('ksampler')) {
            ...
            samplerNodes.push({
                id: Number.isFinite(Number(nodeId))
                    ? Number(nodeId)
        ...
        if (upscaleFactor === undefined && classType.includes('upscale')) {
            upscaleFactor = toFiniteNumber(
                inputs.scale_by || inputs.upscale_by,
            );
        }

        if (!upscaler && classType.includes('upscale')) {
            upscaler =
                typeof inputs.upscale_method === 'string'
    ...
    return {
        prompt:
            normalizePromptText(uniqueText(positiveTexts).join('\n\n')) || '',
        negativePrompt:
            normalizePromptText(uniqueText(negativeTexts).join('\n\n')) || '',
        ...
        vae,
        denoiseStrength:
            upscaleSampler?.denoise ??
            (baseSampler?.denoise !== undefined && baseSampler.denoise < 1
                ? baseSampler.denoise
                : undefined),
    };
}

function parseFallbackMetadata(
    entries: Map<string, string>,
): Partial<ParsedImageMeta> {
    const candidateKeys = [
        'comment',
        'description',
        'usercomment',
    ...
    const fallbackText = candidateKeys
        ...
        .find(Boolean);
    const sourceText =
        fallbackText || normalizePromptText(entries.get('prompt')) || '';
...
function mergeMetadata(
    result: ParsedImageMeta,
    candidate: Partial<ParsedImageMeta>,
): void {
    ...
    for (const key of keys) {
        applyIfMissing({ target: result, source: candidate, key });
    }
...
function pickMetadataFromEntries(
    entries: Map<string, string>,
    warnings: string[],
): ParsedImageMeta {
...
async function readExifEntriesFromBuffer(
    buffer: Buffer,
...
export async function readImageMetadataFromBuffer(
    ...
    if (buffer.length > MAX_IMAGE_FILE_SIZE_BYTES) {
        warnings.push(
            `Image exceeds ${MAX_IMAGE_FILE_SIZE_BYTES} byte size limit`,
        );
        return {
            prompt: '',
            negativePrompt: '',
    ...
    return pickMetadataFromEntries(
        mergeEntries(pngEntries, exifEntries),
        warnings,
    );
}
