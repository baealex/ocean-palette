...
function isBcryptLike(candidate: unknown): candidate is BcryptLike {
    ...
    return (
        typeof value.hash === 'function' && typeof value.compare === 'function'
    );
}
...
async function createPasswordService(): Promise<PasswordService> {
    ...
    return {
        ...
        async verify(password: string, storedHash: string): Promise<boolean> {
            const hasher = hashers.find((candidate) =>
                candidate.supports(storedHash),
            );
            if (!hasher) {
                return false;
            }
...
export const getPasswordHashAlgorithm =
    async (): Promise<PasswordAlgorithm> => {
        const passwordService = await getPasswordService();
